...
const DashboardScreen = (props) => {
  ...
  useEffect(() => {
    ...
    checkDocument();
    checkContact();
    console.log(`declaration : ${isDeclaration_one}, document : ${isDocument1}, contact : ${full_name}, business name : ${business_name}`)

    if (status == "activated") {
      setDashboardDisplay(true)
    } else {
      if (isDeclaration_one != 1) {
        setLink("TAKDE DECLARATION")
        if (isDocument1 == 'http://test') {
          setLink("TAKDE DOCUMENT")
          if (full_name == 'Amirah binti Amir') {
            setLink("TAKDE CONTACT")
            if (business_name == undefined) {
              setLink("TAKDE BUSINESS")
            } else { setLink("ADE BUSINESS") }
          } else { setLink("ADE CONTACT") }
        } else { setLink("ADE DOCUMENT") }
      } else { setLink("ADE DECLARATION") }
    }
